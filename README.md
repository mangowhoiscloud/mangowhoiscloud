...
| **Plan and Execute** | LLM 출력을 받아 다음 행동을 결정하는 오케스트레이션 | AgenticLoop `while(tool_use)`, Hook System(Event Bus), LaneQueue, TaskGraph, bash->tool->skill->MCP Registry, Sub-Agent(MAX_CONCURRENT=5, DAG), Model Switching Context Compression |
