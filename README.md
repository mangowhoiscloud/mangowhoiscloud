...
<p align="center">
  ...
  <a href="https://mangowhoiscloud.github.io/geode/self-improving/">
    <img src="https://img.shields.io/badge/Self--Improving-Hub-6B4FBB?style=flat-square&logo=githubpages&logoColor=white" />
  </a>
</p>
...
프로세스가 늘어나면 언제나 그랬듯 OS가 필요해집니다. 그래서 에이전트를 챗봇이 아니라 OS로 다룹니다 —
도구를 부르는 손, 프로세스를 관리하는 스케줄러, 컨텍스트를 계층화하는 메모리, 위험한 호출 앞에 서는 권한 게이트.

**신뢰는 감정이 아니라 아키텍처입니다.** 에이전트의 자기 보고를 믿지 않아 검증을 에이전트 바깥의 결정론 코드에 둡니다.
평가자도 믿지 않아 생성과 평가의 모델 패밀리를 가르고(cross-family judge), 단일 평가 층도 믿지 않아 직교 다층(Swiss Cheese)으로 겹칩니다.
측정 자체도 믿지 않아 개선 곡선보다 노이즈 밴드와 대조군을 먼저 세웁니다.
